...
class ThingSet_CAN(threading.Thread):
    ...
    def __init__(self, if_name:str, own_id:int):
        super().__init__()
        if own_id not in range(0,256):
            raise ValueError("Address must be integer between 0 and 255 (got: {})".format(own_id))
    ...
    def post(self, addr:int, id:int, data):
        ...
        frame = RequestFrame(src=self.__own_id, dst=addr, data=bytes(frame_data))
        self.__sock.send(frame)
        if self.__waitResponse(0.5):
            ret = self.__getResponse()
        else:
    ...
    def patch(self, addr: int, obj_id: int, map:dict):
        ...
        self.__sock.send(RequestFrame(src=self.__own_id, dst=addr, data=bytes(data)))
